...
import {
  ...
  Grid3x3, List, X, ArrowLeft, ExternalLink, MoreVertical, FolderPlus,
} from 'lucide-react';
...
export default function DocumentsDrive({ initialDocs, projects }: { initialDocs: DocumentRow[]; projects: Project[] }) {
  ...
  const [folder, setFolder] = useState<string>(''); // current folder_path
  const [view, setView] = useState<'grid' | 'list'>('grid');
  const [sort, setSort] = useState<SortKey>('date');
  const [q, setQ] = useState('');
  const [selected, setSelected] = useState<DocumentRow | null>(null); // for preview
  const [menuOpen, setMenuOpen] = useState<string | null>(null); // file id for context menu
  ...
  const visible = useMemo(() => {
    // When searching, search across ALL folders in bucket
    let arr: DocumentRow[];
    if (q.trim()) {
      const s = q.toLowerCase();
      arr = inBucket.filter((d) => d.name.toLowerCase().includes(s));
    } else {
      // Show items in current folder only
      arr = inBucket.filter((d) => (d.folder_path || '') === folder);
    }
    // Sort: folders first, then by sort key
    arr.sort((a, b) => {
  ...
  async function createFolder() {
    ...
    // Insert folder row (is_folder=true) — pick project from filter or first
    ...
    const { error } = await supabase.from('documents').insert({
      ...
      name: safeName,
      bucket,
      path: `_folders/${path}`, // virtual marker
      folder_path: folder,
      is_folder: true,
      uploaded_by: 'web',
    });
  ...
  async function deleteDoc(d: DocumentRow) {
    ...
    if (!d.is_folder) {
      ...
      // Delete all files inside folder (any depth)
  ...
  async function renameDoc(d: DocumentRow, newName: string) {
    if (!newName.trim() || newName === d.name) {
      setRenaming(null);
      return;
    }
  ...
  return (
    <div className="space-y-4" key={refreshKey}>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <select
          value={projFilter}
          onChange={(e) => { setProjFilter(e.target.value); setFolder(''); setTimeout(refresh, 0); }}
      ...
      {(folder || breadcrumb.length > 0) && (
        <div className="flex items-center gap-1 text-xs text-slate-600 flex-wrap">
          <button
            onClick={() => { setFolder(''); setQ(''); }}
            className="flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-slate-100"
          >
            <Home size={12} /> Root
          </button>
          {breadcrumb.map((c, i) => (
            <span key={c.path} className="flex items-center gap-1">
              ...
              <button
                onClick={() => { setFolder(c.path); setQ(''); }}
                className={`px-1.5 py-0.5 rounded hover:bg-slate-100 ${i === breadcrumb.length - 1 ? 'font-semibold' : ''}`}
              >
                {c.name}
              </button>
            </span>
          ...
          {q.trim() && (
            <span className="ml-2 text-slate-400">· searching across all folders</span>
          )}
        </div>
      )}

      {/* Upload dropzone (only in current folder, not when searching) */}
      {!q.trim() && (
        <UploadDropzone
          bucket={bucket}
      ...
      <div className="bg-white rounded-xl p-3 md:p-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-sm">
            {q.trim() ? `Search results (${visible.length})` : `${visible.length} items`}
          </h3>
        </div>
            ...
            <table className="w-full text-xs min-w-[600px]">
              ...
              <tbody>
                {visible.map((d) => (
                  <tr
                    key={d.id}
                    className="border-b border-slate-50 hover:bg-slate-50 group relative"
                    onClick={() => d.is_folder ? openFolder(d) : isPreviewable(d.name) && setSelected(d)}
                  >
      ...
      {renaming && (
        <RenameModal
          ...
          onSubmit={(newName) => renameDoc(renaming, newName)}
        />
      )}
      ...
      {moving && (
        <MoveModal
          ...
          onSubmit={(newFolder) => moveDoc(moving, newFolder)}
        />
      )}
    </div>
  );
...
function PreviewModal({ doc, onClose }: { doc: DocumentRow; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-2 sm:p-4 bg-black/80" onClick={onClose}>
      <div
        className="relative bg-white rounded-xl max-w-4xl w-full max-h-[92vh] overflow-hidden flex flex-col"
...
function MoveModal({ doc, allFolders, onClose, onSubmit }: {
  doc: DocumentRow;
  allFolders: DocumentRow[];
  onClose: () => void;
  onSubmit: (folder: string) => void;
}) {
  // Build list of folder paths (unique)

...
def build_cobie_workbook(
    ...
    # ── Document properties ──────────────────────────────────────────────
    # Stamp the workbook with origin metadata so downloaded COBie XLSX
    # files carry our authorship even if the visible UI strings are
    # localised away. Sits in docProps/core.xml (visible in Excel's File
    # Info pane) and docProps/custom.xml (rarely surfaced in the UI).
    try:

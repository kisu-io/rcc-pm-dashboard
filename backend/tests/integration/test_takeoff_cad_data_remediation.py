...
# fixture above (full app boot + register/login round-trip), which is
# environmental noise unrelated to the 410 contract we want to lock. We
# mount just the takeoff router on a fresh app with overridden auth
# dependencies, backed by a throwaway PostgreSQL database - the same
# trick the ``test_projects_profile_autoretrofit.py`` suite uses.


@asynccontextmanager
async def _build_takeoff_only_app():
    """Boot a minimal FastAPI app with just the takeoff router mounted,
    auth/perm dependencies stubbed, and a per-call empty session on a
    throwaway PostgreSQL database so the database fallback path of
    ``_get_cad_session`` runs cleanly (it will simply find no row →
    return None → endpoint raises 410)."""
...
async def test_cad_data_describe_unknown_session_returns_410() -> None:
    ...
    re-upload hint."""
    async with _build_takeoff_only_app() as app_:
